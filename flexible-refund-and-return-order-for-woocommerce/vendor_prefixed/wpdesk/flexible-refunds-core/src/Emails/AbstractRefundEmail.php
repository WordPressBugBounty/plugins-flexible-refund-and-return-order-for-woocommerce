...
abstract class AbstractRefundEmail extends WC_Email
{
    ...
    public function __construct()
    {
        ...
        $this->template_base = Integration::get_template_path();
    ...
    protected function get_renderer(): Renderer
    {
        ...
        $resolver->appendResolver(new DirResolver(get_stylesheet_directory() . '/flexible-refunds-pro/'));
        $resolver->appendResolver(new DirResolver(Integration::get_library_path() . 'src/Views'));
        return new SimplePhpRenderer($resolver);
    }
    protected function get_settings(): PersistentContainer
    {
        return new WordpressOptionsContainer(Integration::SETTING_PREFIX);
    }
    /**
    ...
    public function generate_wysiwyg_html($key, $data)
    {
        ...
        wp_editor(wp_kses($this->get_option($key), EmailHelper::allowed_tags()), $field_key, $args);
        ?>
					<?php 
